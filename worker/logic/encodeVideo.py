...
def encode_video(task):
  ...
  with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    shutil.copy(tmpdirname + "/output0.m4s", tmpdirname + "/output_" + str(task.encode_video_task.index) + ".m4s")
    # Build output
    if (task.encode_video_task.index == 0):
      upload_file(task.encode_video_task.output_directory, Path(tmpdirname + "/init.mp4"))
    upload = upload_file(task.encode_video_task.output_directory, Path(tmpdirname + "/output_" + str(task.encode_video_task.index) + ".m4s"))
