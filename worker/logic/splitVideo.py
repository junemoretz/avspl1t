...
def split_video(task):
  ...
  with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    cmd = f'ffmpeg -i {tmpdirname}/input.{ext} -c:a copy -c:v h264 -preset ultrafast -crf 17 -map 0:v -map 0:a -sc_threshold 0 -g {secs} -force_key_frames "expr:gte(t, n_forced * {secs})" -segment_time {secs} -f segment {tmpdirname}/segment_%03d.ts'
    subprocess.run(shlex.split(cmd), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) 
    # Iterate over output
    pathlist = Path(tmpdirname).glob('segment*')
    files = []
    for path in natsorted(pathlist):
